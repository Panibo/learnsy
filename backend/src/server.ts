...
export function createApiServer(db: Db, allowedOrigins: string[]) {
  return createServer({ requestTimeout: 30_000, headersTimeout: 15_000 }, async (request, response) => {
    try {
      ...
      if (request.url !== "/api/profile") { json(response, 404, { error: "Not found." }); return; }
      if (request.method === "OPTIONS") {
        response.writeHead(204, { "Access-Control-Allow-Methods": "GET, PUT, OPTIONS", "Access-Control-Allow-Headers": "Authorization, Content-Type", "Access-Control-Max-Age": "600" });
        response.end(); return;
      }
